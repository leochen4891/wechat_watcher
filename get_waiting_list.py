...
def get_lrsg_rooms(chatrooms):
	...
	if not r1 or not r2:
		logger.error("Not found both lrsg room 1 and 2 in the chatroom list, try send or receive a msg in the room to have it appear in the chatroom list, r1 = %s, r2 = %s" % (r1, r2))
		return None
	return [r1, r2]
...
def get_waiting_list():
	...
	with open('waiting_list.json', 'w', encoding='utf-8') as f:
		json.dump([ "%s(%s)" % (member['NickName'], member['DisplayName']) for member in waiting_list], f, ensure_ascii=False, indent = 2)
